...
// tokens, the first succeeds and persists new tokens while the second presents
// the now-consumed token, gets invalid_grant, and the SDK calls
// invalidateCredentials("tokens") — wiping the freshly saved tokens and forcing
// re-auth. A single in-flight lock means the second connect runs only after the
// first has saved fresh tokens, so it reads valid tokens and skips the refresh.
