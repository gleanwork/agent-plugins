...
// Every spec here spawns the real hook against a stubbed host CLI, and the stub is a
// `#!/bin/sh` script, so the suite is POSIX-only. The hook itself is cross-platform --
// claudeCandidates() already handles the Windows `.cmd` shim, and the shipped tree carries
// no shell scripts by CI guard -- it is only the test's stub that cannot be made portable
// without inventing an executable Node shim. CI runs ubuntu, so this skips only for
// developers on Windows, where a failure would otherwise look like a real defect.
const WINDOWS = process.platform === "win32";

const here = path.dirname(fileURLToPath(import.meta.url));
const HOOK_SOURCE = path.resolve(
  here,
  "../../../../overrides/claude/glean/hooks/capture-inventory.mjs",
);
...
describe.skipIf(WINDOWS)("claude mcp list capture", () => {
...
describe.skipIf(WINDOWS)("the Glean-only filter", () => {
...
describe.skipIf(WINDOWS)("the hook and the server agree on where the capture lives", () => {
...
describe.skipIf(WINDOWS)("how the capture is written", () => {
...
describe.skipIf(WINDOWS)("hook preconditions", () => {
