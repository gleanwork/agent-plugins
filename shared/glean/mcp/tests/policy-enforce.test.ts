...
describe("deactivated", () => {
  ...
  it("blames the version, not the features", () => {
    const text = (refusal("find_skills_and_tools", d)!.content[0] as { text: string }).text;
...
describe("the remote's upgrade text", () => {
  it("is used as the remedy in a deactivation refusal", () => {
    const d = decision({
      ...
      features: { toolPromotion: false, metaTools: false, fileArgs: false },
      upgradeMessage: "Run `/plugin update glean` and restart Claude Code.",
    });

    const text = (refusal("find_skills_and_tools", d)!.content[0] as { text: string }).text;
    expect(text).toContain("Run `/plugin update glean`");
  });
